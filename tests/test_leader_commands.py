...
def test_command_history_navigation(tmp_path):
    ...
    # Close the popup generated by the previous command
    handler.handle_key(None, 27)

    _enter_command_mode(handler)
    assert nav.command_history_index == 1
    assert nav.command_buffer == ""

    handler.handle_key(None, 16)
    assert nav.command_history_index == 0
    assert nav.command_buffer == "!echo hi"

    handler.handle_key(None, 14)
    assert nav.command_history_index == 1
    assert nav.command_buffer == ""
    ...
    handler.handle_key(None, ord(" "))
    assert nav.command_buffer == "!echo hi "
    assert nav.command_history_index == len(nav.command_history)
